...
const Comments = () => {
  ...
  const handleComment = (postId) => {
    ...
    parsedComments.push({ text: commentInput, user: 'Admin' });
  ...
  const handleDeleteComment = (postId, index) => {
    ...
    setBlogs(prevBlogs => {
      return prevBlogs.map(blog => {
        if (blog.id === postId) {
          return { ...blog, comments: parsedComments };
        }
        return blog;
      });
    });
  ...
  const handleDeleteBlog = (postId) => {
    ...
    axios.delete(`http://localhost:5122/api/BlogPosts/${postId}`)
      .then(() => {
              ...
              <ul style={{listStyle:"none"}}>
                {JSON.parse(Cookies.get(`comments_${blog.id}`) || '[]').map((comment, index) => (
                  <li key={index}>
                    ...
                    {comment.text}
                    <button onClick={() => handleDeleteComment(blog.id, index)} style={{backgroundColor:"darkred",color:"white"}}>Delete</button>
                  </li>
                ))}
              </ul>
              {/* Delete blog button */}
